...
<body>
	<div class="wrap">
		...
		<div class="content">
			<b class="contentTitle" style="font-size: 18px;">참조/열람문서함</b> <br>
			<br>
			<div class="contentDiv" style="border: 0px;">
				...
				<div style="height:470px;">
					<table id="refTab">
						...
						<c:if test="${ empty boxList }">
							<td colspan="9" rowspan="10"><div style="text-align:center; line-height:90px; color:#a5a5a5;"> 참조/열람할 문서가 없습니다. </div></td>
						</c:if>
						<c:if test="${ ! empty boxList }">
							<c:forEach var="b" items="${ boxList }">
								<tr class="boxListTr">
									...
									<td>
										<c:if test="${b.perKind eq 0 }">
											열람
										</c:if>
										<c:if test="${b.perKind eq 1 }">
											참조
										</c:if>
									</td>
									<td>${ b.docTitle }</td>
				...
				<div id="pagingDiv" align="center">
					<ul class="pagination">
						...
						<c:if test="${ pi.currentPage > 1 }">
							<c:url var="blistBack" value="goAppRef.ap">
						...
						<c:forEach var="p" begin="${ pi.startPage }" end="${ pi.endPage }">
							...
							<c:if test="${ p ne pi.currentPage }">
								<c:url var="blistCheck" value="goAppRef.ap">
						...
						<c:if test="${ pi.currentPage < pi.endPage }">
							<c:url var="blistEnd" value="goAppRef.ap">
							<c:param name="currentPage" value="${ pi.currentPage + 1 }" />
							</c:url>
							<li>
								<a href="${ blistEnd }">
